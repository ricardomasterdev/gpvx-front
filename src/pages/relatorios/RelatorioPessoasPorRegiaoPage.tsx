...
export const RelatorioPessoasPorRegiaoPage: React.FC = () => {
  ...
  const { data: agregacaoData } = useQuery({
    queryKey: ['relatorio-pessoas-por-regiao-agregacao', gabinete?.id],
  ...
  // Mapa de quantidades por regiao (da agregacao - sempre atualizado)
  ...
  // Cidades da regiao selecionada (da agregacao)
  const cidadesDaRegiao = useMemo((): { municipioId: string; nome: string; quantidade: number; percentual: number }[] => {
    if (!regiaoId) return [];
    const agregacaoItems = agregacaoData?.items || [];
    const regiaoSelecionada = agregacaoItems.find((item: any) => item.regiaoId === regiaoId) as RelatorioRegiaoItem | undefined;
    return regiaoSelecionada?.cidades || [];
  }, [regiaoId, agregacaoData]);

  // Opcoes para os selects (com quantidade da agregacao)
  const regioesOptions = useMemo(() => [
    { value: '', label: `Todas as regioes (${totalGeralAgregacao.toLocaleString('pt-BR')})` },
    ...regioes.map((r: any) => {
  ...
  ], [regioes, quantidadesPorRegiao, totalGeralAgregacao]);
